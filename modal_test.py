...
1. comp_demo() - Original comparison app with full feature set (optimized for T4/L4)
2. comp_demo_original() - UNOPTIMIZED original app with both models loaded (no constraints)


Usage:
    # Deploy the UNOPTIMIZED original demo (both models loaded, no constraints)
    python -m modal run modal_test.py::comp_demo_original
...
- Handle memory optimization for T4/L4 GPUs (except comp_demo_original)
"""
...
# Function for comp_demo_original - UNOPTIMIZED original app with both models
@app.function(
    image=inference_image,
...
**Visualizations:** {len(finetuned_vis_images)} (full resolution)"""
    ...
    return {"status": "comp_demo_original app launched successfully"}
